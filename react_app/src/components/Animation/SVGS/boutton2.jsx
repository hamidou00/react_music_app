...
export default function Boutton() {
    ...
    function rotateButton(currentMouseOffsetY){
        ...
        if (mouseDirectionY(currentMouseOffsetY)){
            ...
            if (rotation2.current < 310){
                ...
                // setStroke(((rotation2.current * 100) / 360).toFixed())
    ...
    const onMouseDown = (evt) => {
        if (evt.target.id === "Centre") isRotating2.current = true
        // setIsRotating(true)
        ...
        >
            ...
            <title>Sans titre - 1</title>
                <g id="Button2Test">
                    ...
                    <circle id="Centre" className="cls-4" cx="97.79" cy="99.78" r="70.62"/>
                    <line id="Indicateur" class="cls-5" x1="72.28" y1="154.94" x2="78.92" y2="140.33"/>
                    {/* <line id="Indicateur" className="cls-5" x1="97.53" y1="38.76" x2="97.53" y2="54.81"/> */}
                    </g>
                </g>
            </svg>
            <p>Value (en degré) : {rotation2.current}</p>
            <p>Statut de la souris  : {mouseDirection}</p>
            <input type="range" min="0" max="310" step="1" value={stroke} onChange={slider}/>
            <p>Stroke  : {stroke}</p>
            <p>Stroke en %: {((rotation2.current * 100) / 360).toFixed() }</p>
            <p>Stroke en %: { parseInt(stroke) +14}</p>
            <p>Actual STROKEEEE: { ((parseInt(stroke) * 3) + 14)}</p>
            {/* <p>calcul: { parseInt(((stroke * 100) / 360).toFixed()) + 162 }</p> */}
            <p>calcul: { ((rotation2.current * 534) / stroke).toFixed() }</p>
            <p>conversiont: {( stroke * 180) / Math.PI }</p>
            <p>conversiont: {( 0.35  * 180) / Math.PI }</p>
        </>
    )
}
