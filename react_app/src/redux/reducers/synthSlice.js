...
export const synthSlice = createSlice({
    ...
    reducers: { // Les differentes actions (c ici que je vais modif le volume effects, part, matrix et SURTOUR LE METRONOME MERDE trop bien)
        ...
        // setVibrato: (state, action) => {
        //     // console.log(action.payload.synthIndex)
        //     state.synths[action.payload.synthIndex].effects.vibrato = action.payload.vibrato
        // },

        setOneEffect: (state, action) => {
            // action.effectName
            // action.effectValues
            // action.synthIndex
            switch (action.payload.effectName) {
                case "BitCrusher": state.synths[action.payload.synthIndex].effects.bitCrusher = action.payload.bitCrusher; break;
                case "Vibrato": state.synths[action.payload.synthIndex].effects.vibrato = action.payload.vibrato; break;
                // default:break;
            }
        },
...
export const getVibrato = (state, synthIndex) => state.Synth.synths[synthIndex].effects.vibrato;
export const getFeedBackDelay = (state, synthIndex) => state.Synth.synths[synthIndex].effects.feedBackDelay;
